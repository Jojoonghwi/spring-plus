...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException,
		...
		String email = ((UserDetailsImpl) authResult.getPrincipal()).getUsername();//Username이름에 이메일
		UserRole role = ((UserDetailsImpl) authResult.getPrincipal()).getUser().getUserRole();
		String nickname = ((UserDetailsImpl) authResult.getPrincipal()).getNickname();//Username이름에 이메일
